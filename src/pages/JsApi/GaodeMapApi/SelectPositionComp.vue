<template>
  <div id="SelectPosition" :class="{ visible: visible }">
    <!-- 返回/取消按钮 -->
    ...
    <div
      class="confirm-btn"
      :class="{ active: currSelectedAddress }"
...
export default {
  ...
  methods: {
    ...
    resetInitPosition() {
      if (this.initCenter && this.initLocation) {
        this.searchServer2.searchNearBy(
          ...
          (status, result) => {
            ...
            if (!poiList || !poiList.pois.length) {
              // 无法定位到准确位置,则定位到视野
    ...
    // 确认/保存
    confirmAddress() {
      if (this.currSelectedAddress && !this.currSelectedAddress.basicInfo) {
        this.$emit("change", this.currSelectedAddress);
      }

      // 补全 autocomplete 的地址信息
      if (this.currSelectedAddress && this.currSelectedAddress.basicInfo) {
        this.searchServer2.getDetails(
          this.currSelectedAddress.id,
          (status, result) => {
            const { poiList } = result;
